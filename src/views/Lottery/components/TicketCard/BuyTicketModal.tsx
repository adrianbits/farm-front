  ...
  return (
    <Modal title={t('Enter amount of tickets to buy')} onDismiss={onDismiss}>
      <TicketInput
        ...
        availableSymbol="TENDIE"
      />
      <div>
        <Tips>{t('1 Ticket = %lotteryPrice% TENDIE', { lotteryPrice: LOTTERY_TICKET_PRICE })}</Tips>
      </div>
      <div>
        <Announce>
          {t('Ticket purchases are final. Your TENDIE cannot be returned to you after buying tickets.')}
        </Announce>
        <Final>{t('You will spend: %num% TENDIE', { num: cakeCosts(val) })}</Final>
      </div>
      <ModalActions>
